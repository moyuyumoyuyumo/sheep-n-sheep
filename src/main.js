...
import { setupCheat } from './ui/cheat.js';

/** 从 wallet 拽一份道具余额快照到 state，render 读这个。 */
function syncToolUses() {
  state.toolUses = getBalance();
}
...
function startLevel(level) {
  ...
  syncToolUses();                                 // 从 wallet 拽余额进 state
...
bindControls(state, {
  ...
  startLevel,
  showMenu,
  syncToolUses,                                  // controls 里花道具 / 领礼包 后调一下
});

// Konami 后门：输完 ↑↑↓↓←→←→BA 或控制台敲 cheat() → 999 道具
setupCheat(() => {
  syncToolUses();
  render(state);
});
...
if (DEBUG) {
  console.log('%c[咩了个咩] 阶段 6 · 道具钱包 + 签到 + 广告 + 后门', 'color:#92400e;font-weight:bold;');
  console.log('  · 槽位数量:', SLOT_CAPACITY, '/ 凑齐消除:', MATCH_COUNT);
  console.log(`  · 共 ${allLevels.length} 个关卡:`, allLevels.map(l => l.name).join(' / '));
  console.log('%c  · 后门：Konami 上上下下左右左右BA 或 window.cheat() → 999 道具', 'color:#fb923c;');
  ...
  console.log('%c  · 已暴露：state, resetState(), rerender(), startLevel(level), showMenu(), cheat()', 'color:#78716c;');
}
